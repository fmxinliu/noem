...
public:
	...
	int		Run_UpChar(int p_nRamBufferID, BYTE* p_pbyTemplate, unsigned int *p_pnSize);
	int		Run_DownChar(int p_nRamBufferID, BYTE* p_pbyTemplate, unsigned int p_nSize);
	...
	int		Run_SetRTC(ST_RTC_TYPE p_stRTCInfo);
	int		Run_GetRTC(ST_RTC_TYPE *p_stRTCInfo);

	int		Run_GetOEMRSAPubKey(BYTE* p_pCryptRemoteN, BYTE* p_pCryptRemoteE, int *p_nKeySizeByte);
	int		Run_SetHostRSAPubKey(BYTE* p_pCryptN, BYTE* p_pCryptE, int p_nKeySizeByte);
