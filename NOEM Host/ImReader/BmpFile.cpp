...
)
{
	...
	*width=0; *height=0;
	...
	if (err != 0) {
		strcpy_s(m_errorText, 255, "Can't open file for reading :\n");
		strcat_s(m_errorText, 255, fileName);
		return NULL;
	} else {
	    long rc;
		rc=fread((unsigned char  *)&(m1),1,1,fp); m_bytesRead+=1;
		if (rc==-1) {strcpy_s(m_errorText, 255, "Read Error!"); fclose(fp); return NULL;}

		rc=fread((unsigned char  *)&(m2),1,1,fp); m_bytesRead+=1;
		if (rc==-1) strcpy_s(m_errorText, 255, "Read Error!");
		if ((m1!='B') || (m2!='M')) {
			strcpy_s(m_errorText, 255, "Not a valid BMP File");
			fclose(fp);
			return NULL;
        }
		...
		if (compression!=BI_RGB) {
	    	strcpy_s(m_errorText, 255, "This is a compressed file.");
	    	fclose(fp);
	    	return NULL;
	    }
		...
		case 8:
			...
			if (colormap==NULL) {
				fclose(fp);
				strcpy_s(m_errorText, 255, "Out of memory");
				return NULL;
			}
			...
			for (i=0;i<colors;i++) {
				...
				m_bytesRead++;
				if (rc!=1) {
					strcpy_s(m_errorText, 255, "Read Error!");	
		...
		if ((long)m_bytesRead>pixoff) {
			fclose(fp);
			strcpy_s(m_errorText, 255, "Corrupt palette");
			delete []colormap; // free_mem(colormap);
			fclose(fp);
			return NULL;
		}
		...
		*height=h;
		...
		if (outBuf==NULL) {
			strcpy_s(m_errorText, 255, "Memory alloc Failed");
		} else {
...
)
{
	...
	if (err != 0) {
		strcpy_s(m_errorText, 255, "Can't open file for writing");
		return 0;
	}
	...
	for (row=0;row<header.biHeight;row++) {
		...
		if (rc!=1) {
			strcpy_s(m_errorText, 255, "fwrite error.");
			break;
		}
